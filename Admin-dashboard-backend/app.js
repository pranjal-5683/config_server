...
app.listen(5000, () => {
  // here listen is function of express js which know the node server is started or not
...
app.post("/getuser", async (req, res) => {
  ...
  try {
    ...
    const exceptpass = await User.findOne({ email: useremail }).select(
      "-password"
    );
    res.send(exceptpass);
    // .then((data)=>{
...
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    cb(null, "../React-Landing-Page-Template-master/src/images/");
  },
  filename: function (req, file, cb) {
    const uniquesuffux = Date.now();
    cb(null, uniquesuffux + file.originalname);
