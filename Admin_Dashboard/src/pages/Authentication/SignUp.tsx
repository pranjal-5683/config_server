...
// Onchange Full Name Textbox code
  const onChangefname=(e: React.FormEvent<HTMLInputElement>)=>{
          let newName = e.currentTarget.value;
          setFname(newName)
          const isValidName = /^[A-Za-z\s]+$/.test(e.currentTarget.value);
          setNameValid(isValidName)
  }
// Onchange Designation Textbox code
  const onChangelname=(e: React.FormEvent<HTMLInputElement>)=>{
    const newName = e.currentTarget.value;
    setLname(newName);
    const isValidName = /^[A-Za-z\s]+$/.test(e.currentTarget.value);
    setLNameValid(isValidName);
}
// Onchange Email Textbox code
const onChangeemail=(e: React.FormEvent<HTMLInputElement>)=>{
  setStatus(null)
  setEmail(e.currentTarget.value);
  const emailPattern = /^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{3,}$/i;
  const valid = emailPattern.test(e.currentTarget.value);
    setIsemailValid(valid);
}
// Onchange Password Textbox code
...
// Onchange Image code
  const onChangeimage=(e: React.FormEvent<HTMLInputElement>)=>{
    ...
    if (file) {
      // Check image type (allowed types: jpg, jpeg, png)
      const allowedTypes = ['image/jpeg', 'image/jpg', 'image/png'];
      if (!allowedTypes.includes(file.type)) {
        setValidationMessage('Invalid file type. Please select a JPG or PNG image.');
        return;
      }
      const maxSizeInBytes = 20 * 1024 ; // 20 KB if you want more size 1*1020*1024; use this for 1 MB
      if (file.size > maxSizeInBytes) {
        setValidationMessage('Image size exceeds the maximum allowed (20 KB).');
        return;
      }
      setImage(file);
  ...
  const handleSubmit=(e: React.FormEvent<HTMLFormElement>)=>{
        e.preventDefault();
        if(namevalid && lnamevalid && isemailValid && valid && validationMessage)
        {
              ...
              .then((data)=>{
                ...
                if(data.status==='User Craeted...'){
                  sweetAlert.fire("User Created...",'','success');
                setFname('');
                setLname('');
                setEmail('');
                setPassword('');
          ...
          document.getElementById("sfname").innerHTML="<span><mark>Name must be Characters</mark></span>"
          document.getElementById("slname").innerHTML="<span><mark>Designation must be Characters</mark></span>"
          document.getElementById("semail").innerHTML="<span><mark>Email must be as <b>xyz@gmail.com</b> format</mark></span>"
          document.getElementById("spass").innerHTML="<span><mark>Password must be at least 8 characters long and contain at least one uppercase letter, one lowercase letter, one number, and one special character.</mark></span>"
          document.getElementById("pimage").style.backgroundColor='yellow';
        }
  }
              ...
              <form onSubmit={(e) =>handleSubmit(e)}>
                <div className="mb-4">
                  ...
                  <div className="relative">
                    <input autoFocus={true} required style={namevalid? {border:'1px solid green'}:namevalid==null? {border:'1px solid blue'}:{border:'1px solid red'}} 
                      type="text" name='fname' id='fname' onChange={onChangefname} value={fname}
                      placeholder="Enter your full name"
                      className="w-full rounded-lg border border-stroke bg-transparent py-4 pl-6 pr-10 outline-none focus:border-primary focus-visible:shadow-none dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary"
                    />
                 
        <span id='sfname' style={namevalid? {display:'none'}:namevalid==null? {display:'none'}:{display:'block'}} className="absolute right-1 top-15 text-[#dc2626] ">
         Name must be Characters
                ...
                <div className="mb-4">
                  ...
                  <div className="relative">
                    <input required style={lnamevalid? {border:'1px solid green'}:lnamevalid==null? {border:'1px solid blue'}:{border:'1px solid red'}}
                      type="text" name='lname' id='lname' onChange={onChangelname} value={lname}
                      placeholder="Enter your designation"
                      className="w-full rounded-lg border border-stroke bg-transparent py-4 pl-6 pr-10 outline-none focus:border-primary focus-visible:shadow-none dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary"
                    />
<span id='slname' style={lnamevalid? {display:'none'}:lnamevalid==null? {display:'none'}:{display:'block'}} className="absolute right-1 top-15 text-[#dc2626] ">
         Designation must be Characters
                ...
                <div className="mb-4">
                  ...
                  <div className="relative">
                    <input type="file" accept=".jpg, .jpeg, .png" style={validationMessage.length==1 ? {border:'1px solid blue'}:validationMessage.length > 2 ? {border:'1px solid red'}:{border:'1px solid green'}}
                      name='image' id='image' onChange={onChangeimage}
                      placeholder="Enter your Last name"
                      className="w-full rounded-lg border border-stroke bg-transparent py-4 pl-6 pr-10 outline-none focus:border-primary focus-visible:shadow-none dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary"
                    />
           
          {validationMessage && <p id='pimage' className="absolute right-1 top-17 text-[#dc2626] ">{validationMessage}</p>}
            ...
            {image=='' || image==null ? " ": <img src={image} width='50px' height='50px'/> }
                ...
                <div className="mb-4">
                  ...
                  <div className="relative">
                    <input required style={isemailValid? {border:'1px solid green'}:isemailValid==null? {border:'1px solid blue'}:{border:'1px solid red'}}
                      type="taxt" name='email' id='email' onChange={onChangeemail} value={email}
                      placeholder="Enter your email"
                      className="w-full rounded-lg border border-stroke bg-transparent py-4 pl-6 pr-10 outline-none focus:border-primary focus-visible:shadow-none dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary"
                    />

                    <span id='semail' style={isemailValid? {display:'none'}:isemailValid==null? {display:'none'}:{display:'block'}} className="absolute right-1 top-15 text-[#dc2626] ">
                      Email must be as <b>xyz@gmail.com</b> format
                        </span>
                      {status? <span className="absolute right-1 top-15 text-[#dc2626]">This email is already registred</span>:''}
                ...
                <div className="mb-4">
                  ...
                  <div className="relative">
                    <input required style={valid? {border:'1px solid green'}:valid==null? {border:'1px solid blue'}:{border:'1px solid red'}}
                     type={!flag? "text":"password"} name='password' id='password' onChange={handlePasswordChange} value={password}
                      placeholder="Enter your password"
                      className="w-full rounded-lg border border-stroke bg-transparent mb-16 py-4 pl-6 pr-10 outline-none focus:border-primary focus-visible:shadow-none dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary"
                    />
        <span id='spass' style={valid? {display:'none'}:valid==null? {display:'none'}:{display:'block'}} className="absolute right-1 top-15 text-[#dc2626]">
          Password must be at least 8 characters long and contain at least one
          uppercase letter, one lowercase letter, one number, and one special
          character.
        </span>

                    <span onClick={()=>setFlag(!flag)} className="absolute right-4 top-4">
                      {flag ? <img src='https://www.pngitem.com/pimgs/m/600-6000108_modify-the-password-small-eyes-eye-password-png.png' width='22' height='22'/> : <img src='https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQuuSKxQJR0WVeIcojTfXtjd8VJ4-LtJpebFWJoc01cH0B1lh2GIginvgjmqU1DkpJdCyU&usqp=CAU' width='22' height='22'/>} 
