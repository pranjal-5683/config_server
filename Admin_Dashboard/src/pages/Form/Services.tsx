...
const Services = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const result = await axios
      .post('http://localhost:5000/servicesadd', formData, {
        headers: {
      ...
      .then((response) => {
        ...
        if (response.statusText === 'OK') {
          sweetAlert.fire('Service Added Successully', '', 'success');
          getSelectData('');
          setTitle('');
          setDescription('');
        } else {
  ...
  const handleUpdate = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const result = await axios
      .put(`http://localhost:5000/updateservices/${id}`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
          Accept: 'application/json',
        },
      ...
      .then((response) => {
        sweetAlert.fire('Service Updated Successully', '', 'success');
        console.log(response);
        setUtitle('');
        setUdescription('');
      });
  ...
  const handleDelete = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const result = axios
      .delete(`http://localhost:5000/deleteservice/${did}`)
      .then((response) => {
        console.log(response);
        if (response.statusText === 'OK') {
          ...
          <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
            ...
            <form onSubmit={handleUpdate}>
              <div className="p-6.5">
                ...
                <div className="mb-4.5">
                  <label className="mb-2.5 block text-black dark:text-white">
                    Select Service Icon <span className="text-meta-1">*</span>
                  </label>
                  <input
                    type="file"
                ...
                <div className="mb-4.5">
                  ...
                  <input
                    ...
                    required
                    placeholder="Update your Service Title"
                    className="w-full rounded border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary"
                  />
                </div>
                <div className="mb-4.5">
                  <label className="mb-2.5 block text-black dark:text-white">
                    Service Description <span className="text-meta-1">*</span>
                  </label>
                  <textarea
                    rows={6}
                    value={udescription}
                    ...
                    required
                    placeholder="Update Your Service Decription"
                    className="w-full rounded border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary"
                  />
                </div>
              ...
              <div className="p-6.5">
                ...
                <div className="mb-4.5">
                  ...
                  <textarea
                    ...
                    value={description}
                    required
                    placeholder="Enter Your Service Decription"
                    className="w-full rounded border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary"
                  />
                </div>
